...
export async function generateMetadata(): Promise<Metadata> {
  ...
  return {
    title: `eSIM за Великобритания на цени от ${lowestPrice}лв: Мобилен интернет без роуминг`,
    ...
    openGraph: {
      locale: 'bg_BG',
      type: 'website',
      title: `eSIM за Великобритания на цени от ${lowestPrice}лв: Мобилен интернет без роуминг | Travel eSIM`,
      description: `Купи eSIM за Великобритания от ${lowestPrice}лв. Бърз интернет без роуминг такси. Моментална активация с QR код. Работи в цяла Великобритания с 4G/5G покритие.`,
...
export default async function UKPage() {
  ...
  const priceRange = await getPriceRangeInBGN('GB');
  
  // Generate Saily checkout URL for UK
  const checkoutUrl = await generateCountryCheckoutUrl('GB');
  
  // Breadcrumb Schema for UK page
  const breadcrumbSchema = generateCountryBreadcrumbSchema('Великобритания', 'esim-velikobritania');
  
  // Product Schema for UK page
  const productSchema = generateProductSchema({
    name: 'eSIM за Великобритания – Бърз мобилен интернет без роуминг',
    description: 'Купи eSIM за Великобритания с бърз и надежден мобилен интернет. Без физическа SIM карта. Без дългосрочни договори. Моментална активация.',
    url: generateCanonicalUrl('/esim-velikobritania'),
    countryCode: 'GB',
    offers: {
